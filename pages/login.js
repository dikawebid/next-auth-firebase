		...
		signInWithEmailAndPassword(email, password)
			.then((authUser) => {
				let user = {
					...
					emailVerified: authUser?.user?.emailVerified ?? false,
				};
				Cookies.set("user", user, { expires: 1 });
				router.push("/dashboard");
			})
			.catch((error) => {
				onLoginFail(error.message);
			});
	};

	// UI
	const onLoginFail = (description) => {
		toast({
			description: description ?? "Login Fail",
			status: "error",
			position: "top-right",
			duration: 3000,
			isClosable: true,
